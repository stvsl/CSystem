                                      ...
                                      ui(new Ui::DataView)
{
    ...
    ui->fluxTable->horizontalHeader()->setSectionResizeMode(9, QHeaderView::ResizeToContents);
    ui->fluxTable->horizontalHeader()->setSectionResizeMode(10, QHeaderView::ResizeToContents);
...
void DataView::initTable()
{
    ...
    for (int i = 0; i < CSystemMain::nodePro_temp->size(); i++)
    {
        ...
        for (int j = 0; j < 35; j++)
        {
            ui->proTable->item(i, j)->setTextAlignment(Qt::AlignCenter);
        }
    }
}

void DataView::on_pushButton_8_clicked()
{
    ui->fluxTable->clearContents();
    ui->proTable->clearContents();
    for (int i = 0; i < CSystemMain::nodeinfluxData_temp->size(); i++)
